...
E agora? Teremos que programar nossa própria função que calcula o log toda vez que precisarmos tirar o logaritmo de um valor? Para esse caso específico, a resposta é /não/. Mesmo não sendo uma função embutida em Python, podemos obter uma função que calcula o log usando bibliotecas! Veja o exemplo abaixo.
...
*[A]* Vamos investigar o que o módulo ~math~ tem! Rode o código abaixo e verá todos os objetos contidos no módulo ~math~ (veja que é a função ~dir~ que está listando esses objetos para nós). Leia o nome de todos os objetos e escolha 3 que achar interessante para saber mais (ignore os objetos que começam e terminam com dois sublinhados "__"). Antes de mais nada, identifique o tipo do objeto que escolheu com a função ~type~. Se seu objeto for uma função, rode o comando ~help~ para saber mais da função. Se seu objeto for uma variável, dê um ~print~ nela para saber seu valor. Comente brevemente sobre possíveis aplicações científicas dos objetos que escolheu.
...
*[B]* Usando a biblioteca ~math~, faça o Python checar se $\sen{}^2(x) + \cos{}^2(x) = 1$ para 5 valores diferentes de $x$ a sua escolha. Comente caso encontre algo estranho.
...
*[Desafio]* Você já se perguntou o que seria o fatorial de um número não-inteiro? Se sim, então você está se sentindo como Daniel Bernoulli se sentiu há quase 300 anos. A função Gama ($\Gamma$) é o que chamamos de extensão da função fatorial e ela funciona para números não-inteiros! Sabendo que $\Gamma(n+1) = n!$, qual seria o fatorial de $\pi$? Use o módulo ~math~ para este cálculo. _Dica_: a letra Gama em inglês se escreve Gamma, com dois "m".
...
Perceba que ao importar a função ~log~ dessa forma, ela agora funciona sem a necessidade de invocar o ~namespace~ do módulo ~math~ na linha 4. Inclusive, neste caso se escrevermos "~math.log(valor)~" irá dar um erro pois o módulo ~math~ não foi importado (tente e verá)!! O que aconteceu na linha 1 foi que importamos apenas uma função específica desse módulo _e nada mais_.

*[A]* Sua tarefa é calcular o seno de um número aleatório. Para isso, importe apenas as funções necessárias dos módulos ~math~ e ~random~ usando a estrutura de importação que acabamos de ver. _Dica_: a instrução ~import~ deve ser escrita para cada biblioteca que você quer importar.
...
* Lendo, manipulando e graficando dados
...
Antes de seguir em frente, vamos ver uma nomenclatura. O objeto ~dados~ que criamos usando a função ~read_excel~ do ~pandas~ é chamado de ~DataFrame~, que literalmente em português traduzido como um "quadro de dados", mas podemos chamar simplesmente de "tabela". Cheque se isso é verdade rodando o código abaixo.
...
*[C]* Usando a sintaxe que acabamos de ver, crie uma cópia da variável ~dados~ e nesta cópia faça com que a coluna ~População~ armezene a população em unidade de milhão de pessoas.
...
*[D]* Teste seus conhecimentos novos! Crie uma nova planilha de dados com os dados que você quiser (pode buscar uma tabela pronta ou inventar a sua). Use o ~pandas~ para criar um objeto do tipo ~DataFrame~ com essa tabela. De um ~print~ na sua tabela e faça um plot com ela. Salve seu plot em um arquivo.
