...
Na linha 6 vemos que não é necessário apresentar os argumentos nomeados seguindo a mesma ordem dos argumentos assim como definida na função. Isso funciona desde que todos os argumentos tenham um valor atribuido no momento de execução da função.
...
A linha 8 parece similar a linha 7, porém ela acusa um erro quando executada!! Isso porque não é permitido argumentos posicionais /após/ argumetos nomeados. Se você for utilizar uma mistura de argumentos posicionais com nomeados, certifique-se que os argumentos nomeados estão todos após os argumentos posicionais.

As linhas 9 e 10 mostram que precisamos garantir que todos os argumentos tenham valores atribuidos, do contrário a função não executa.
